...
cities = cities %>% 
  mutate(dist_to_state = st_distance(., usboundaries_com),
         ...
         dist_to_bor = st_distance(., wldboundaries_un),
...
#### Distance to USA Coastal Border
```{r}
...
table1 = kable(top5far_cites,
             col.names = c("City", "State", "Distance from Coast"),
             caption = "Top 5 Cities Furthest from a Coastal Border") %>% 
...
ggplot() +
  ...
  ggrepel::geom_label_repel(
    ...
    stat = "sf_coordinates",
    size = 3) + 
  labs(title = "City Distances from a Coastal Border",
       caption = "Showing cities furthest from a Coastal border")
```
...
ggplot() + 
  ...
  geom_sf(data = top5state_map, color = "red", size = .75) + 
  geom_sf(data = usboundaries_com) +
  geom_sf(data = cities_big, color = "green", size = 1) + 
  ggthemes::theme_map() +
  ggrepel::geom_label_repel(
    data = top5state_map, 
    aes(label = city, geometry = geometry), 
...
ggplot() + 
  ...
  geom_sf(data = cities_big, color = "steelblue") + 
...
```{r, message=FALSE, echo=FALSE}
#kable which cities are in the zone
```
